...
class TestOrderBook(unittest.TestCase):
    ...
    def test_handle_amend(self):
        ...
        self.orderbook.handle_order(order1)

        self.assertEqual(self.orderbook.list_bids[2]['id'], 1)
        self.assertEqual(self.orderbook.list_bids[2]['quantity'], 5)
    ...
    def test_handle_delete(self):
        ...
        self.assertEqual(len(self.orderbook.list_bids), 3)
        self.orderbook.handle_order(order1)
        self.assertEqual(len(self.orderbook.list_bids), 2)
    ...
    def test_generate_book_event(self):
        ...
        self.assertEqual(ob_for_aapl.handle_order(order1),
                         {'bid_price': 219, 'bid_quantity': 10, 'offer_price': -1, 'offer_quantity': -1})
        order2 = order1.copy()
        order2['id'] = 2
        order2['price'] = 220
        order2['side'] = 'ask'
        self.assertEqual(ob_for_aapl.handle_order(order2),
                         {'bid_price': 219, 'bid_quantity': 10, 'offer_price': 220, 'offer_quantity': 10})
